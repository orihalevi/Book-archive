    ...
    def load_archive(self):
        ...
        for book_in_archive in books_in_archive():
            ...
            if (
                ...
                line_edits = []
                # A loop that goes through every detail in the dictionary in every book and puts it inside QLineEdit
                ...
                # The variable "I" is the content in line_edit in the numeric place line_edit
                for i in range(len(line_edits)):
                    g = i
                    i += 1
                    # Creating a floating tip when the mouse cursor is placed on the box
                    line_edits[-i].setToolTip(str(self.books_details_groupBoxes[g].title()) + ": " + str(line_edits[-i].text()))
                    if line_edits[-i].text() == "":
                        line_edits[-i].setToolTip(
                            str(self.books_details_groupBoxes[g].title()) + ": ריק")
        ...
        # Checking whether all the books that are displayed are also selected, and if so, then change select_all_check_boxes to Checked, otherwise, to Unchecked
        if self.multiple_books_editing_mode:
            All_book_IDs_of_the_books_displayed = [book["book_id"] for book in self.all_books_in_display["books"]]
            All_the_book_IDs_of_the_selected_books = [book["book_id"] for book in self.selected_books_dictionary["books"]]
            if set(All_book_IDs_of_the_books_displayed).issubset(All_the_book_IDs_of_the_selected_books):
...
if __name__ == "__main__":
    ...
    window = MainWindow()
    icon = QIcon(r"dependence\images\Software icon.png")  # החלף את "path_to_your_icon_file.png" בנתיב לקובץ האיקון שלך
